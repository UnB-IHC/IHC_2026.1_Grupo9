<!-- O título principal (h1) usa a tag de cabeçalho e um emoji para quebrar a frieza do texto -->
# ✅ CheckAAA

<!-- O blockquote (>) funciona como um subtítulo de impacto, resumindo o projeto em uma linha -->
> Um guia e checklist prático para implementação de acessibilidade (A11y) no ciclo de vida do desenvolvimento de software.

<!-- Estes são "Badges" (escudos). Eles dão um visual extremamente profissional e indicam o status e o contexto do repositório logo de cara -->
![Status](https://img.shields.io/badge/Status-Em%20Desenvolvimento-green)
![Disciplina](https://img.shields.io/badge/Disciplina-IHC-blue)
![Universidade](https://img.shields.io/badge/Universidade-UnB-red)

## 📖 Introdução

A acessibilidade digital não deve ser uma etapa final, mas um requisito presente em toda a engenharia do projeto. Este repositório tem como propósito fornecer um checklist prático e aplicável de acessibilidade para projetos de tecnologia. 

O material foi estruturado para guiar diferentes papéis dentro de uma equipe, garantindo que a conformidade seja atingida de forma colaborativa.

## 🎯 Áreas de Abrangência

<!-- Listas com marcadores (bullet points) e termos em negrito facilitam a leitura dinâmica do usuário (scannability) -->
Nossos checklists cobrem as principais áreas do desenvolvimento:

*   **💻 Desenvolvimento Web:** Boas práticas de código, semântica HTML, navegação por teclado, ARIA e contraste.
*   **🎨 Design:** Diretrizes para interfaces inclusivas, hierarquia visual e legibilidade.
*   **📝 Geração de Conteúdo:** Textos alternativos, linguagem simples e estruturação de mídia.
*   **📊 Gestão de Projetos:** Inserção de acessibilidade nos requisitos do projeto, critérios de aceite e acompanhamento ágil.

## 🎓 Contexto Acadêmico

O **CheckAAA** é um projeto criado a partir dos estudos realizados na disciplina de **Interação Humano Computador (IHC)**.

*   **Instituição:** Universidade de Brasília (UnB)
*   **Docente:** Profa. Rejane Maria da Costa Figueiredo

---
<!-- A linha horizontal (---) cria uma separação visual limpa para o rodapé do documento -->
*Desenvolvido pela equipe do Grupo 9 - IHC (UnB).*

## Contribuidores

<table>
  <tr>
    <td align="center"><a href="https://github.com/joaoguilherme14"><img style="border-radius: 50%;" src="https://github.com/joaoguilherme14.png" width="100px;" alt=""/><br /><sub><b>João Guilherme </b></sub></a><br />
    <td align="center"><a href="https://github.com/joaoleless"><img style="border-radius: 50%;" src="https://github.com/joaoleless.png" width="100px;" alt=""/><br /><sub><b>joao leles</b></sub></a><br />   
    <td align="center"><a href="https://github.com/Gotc2607"><img style="border-radius: 50%;" src="https://github.com/Gotc2607.png" width="100px;" alt=""/><br /><sub><b>Giovani </b></sub></a><br />   
    <td align="center"><a href="https://github.com/DaviUrsulino "><img style="border-radius: 50%;" src="https://github.com/DaviUrsulino.png" width="100px;" alt=""/><br /><sub><b>Davi Ursulino</b></sub></a><br />
    <td align="center"><a href="https://github.com/Luizz97 "><img style="border-radius: 50%;" src="https://github.com/Luizz97.png" width="100px;" alt=""/><br /><sub><b>Luiz Henrique</b></sub></a><br />
    <td align="center"><a href="https://github.com/davirnunes"><img style="border-radius: 50%;" src="https://github.com/davirnunes.png" width="100px;" alt=""/><br /><sub><b>Davi Nunes</b></sub></a><br />
    <td align="center"><a href="https://github.com/arthurgomes1290"><img style="border-radius: 50%;" src="https://github.com/arthurgomes1290.png" width="100px;" alt=""/><br /><sub><b>artur gomes</b></sub></a><br />
   
  </tr>
</table>
